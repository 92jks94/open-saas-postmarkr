...
async function handleMailPaymentCompleted(session) {
    try {
        ...
        await confirmMailPaymentService(session.id, // Use session ID as payment intent ID
        mailPieceId, { entities: { MailPiece: require('wasp/server').prisma.mailPiece, MailPieceStatusHistory: require('wasp/server').prisma.mailPieceStatusHistory } });
