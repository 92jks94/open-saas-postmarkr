...
export default function AddressManagementPage() {
    ...
    const handleDelete = async (addressId) => {
        try {
            ...
            setAddressError(error instanceof Error ? error.message : 'Failed to delete address. Please try again.');
        }
    };
    // Edit handlers
    ...
    const handleEditSave = async (addressId) => {
        try {
            ...
            setAddressError(error instanceof Error ? error.message : 'Failed to update address. Please try again.');
        }
    };
    ...
    const handleCreateAddress = async (e) => {
        try {
            ...
            const addressTypeValue = formData.get('addressType') || 'both';
            const validAddressTypes = ['sender', 'recipient', 'both'];
            const addressType = validAddressTypes.includes(addressTypeValue)
                ? addressTypeValue
                : 'both';
            const addressData = {
            ...
            setAddressError(error instanceof Error ? error.message : 'An unexpected error occurred while creating the address.');
        }
        finally {
            setIsCreating(false);
        }
          ...
          <CardContent className='space-y-8 my-10 py-8 px-6 mx-auto sm:max-w-2xl'>
                ...
                <div className='space-y-4'>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                    <div className='space-y-2'>
                      ...
                      <Input id='contactName' name='contactName' placeholder='John Doe' required onChange={() => {
            setAddressError(null);
            if (formErrors.contactName) {
                setFormErrors(prev => ({ ...prev, contactName: '' }));
            }
        }} className={cn('h-10', formErrors.contactName && 'border-red-500')}/>
                      {formErrors.contactName && (<p className='text-sm text-red-600'>{formErrors.contactName}</p>)}
                    </div>
                    
                    <div className='space-y-2'>
                      <Label htmlFor='companyName' className='text-sm font-medium'>Company Name</Label>
                      <Input id='companyName' name='companyName' placeholder='Acme Corp' onChange={() => setAddressError(null)} className='h-10'/>
                    </div>
                  </div>
                </div>
                ...
                <div className='space-y-4'>
                  ...
                  <div className='space-y-4'>
                    <div className='space-y-2'>
                      ...
                      <Input id='addressLine1' name='addressLine1' placeholder='123 Main St' required onChange={() => {
            setAddressError(null);
            if (formErrors.addressLine1) {
                setFormErrors(prev => ({ ...prev, addressLine1: '' }));
            }
        }} className={cn('h-10', formErrors.addressLine1 && 'border-red-500')}/>
                      {formErrors.addressLine1 && (<p className='text-sm text-red-600'>{formErrors.addressLine1}</p>)}
                    </div>

                    <div className='space-y-2'>
                      <Label htmlFor='addressLine2' className='text-sm font-medium'>Address Line 2</Label>
                      <Input id='addressLine2' name='addressLine2' placeholder='Suite 100, Apartment 2B' onChange={() => setAddressError(null)} className='h-10'/>
                    </div>

                    <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                      <div className='space-y-2'>
                        <Label htmlFor='city' className='text-sm font-medium'>City *</Label>
                        <Input id='city' name='city' placeholder='New York' required onChange={() => {
            setAddressError(null);
            if (formErrors.city) {
                setFormErrors(prev => ({ ...prev, city: '' }));
                      ...
                      <div className='space-y-2'>
                        ...
                        <Input id='state' name='state' placeholder='NY' required onChange={() => {
            setAddressError(null);
            if (formErrors.state) {
                setFormErrors(prev => ({ ...prev, state: '' }));
                      ...
                      <div className='space-y-2'>
                        ...
                        <Input id='postalCode' name='postalCode' placeholder='10001' required onChange={() => {
            setAddressError(null);
                ...
                <div className='space-y-4'>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                    <div className='space-y-2'>
                      ...
                      <Select name='country' required onValueChange={() => {
            setAddressError(null);
            if (formErrors.country) {
                setFormErrors(prev => ({ ...prev, country: '' }));
                        ...
                        <SelectContent>
                          {SUPPORTED_COUNTRIES.map((country) => (<SelectItem key={country} value={country}>
                              {country}
                            </SelectItem>))}
                        </SelectContent>
                      </Select>
                    ...
                    <div className='space-y-2'>
                      ...
                      <Select name='addressType' defaultValue='both'>
                        ...
                        <SelectContent>
                          {ADDRESS_TYPES.map((type) => (<SelectItem key={type} value={type}>
                  ...
                  <div className='space-y-2'>
                    ...
                    <Input id='label' name='label' placeholder='Home, Office, Client ABC' onChange={() => setAddressError(null)} className='h-10'/>
                  </div>
                </div>

                {/* Submit Button with better styling */}
                <div className='pt-4'>
                  <Button type='submit' disabled={isCreating} className='w-full h-11 text-base font-medium' size='lg'>
                    {isCreating ? 'Creating Address...' : 'Create Address'}
                  </Button>
                </div>

                {addressError && (<Alert variant='destructive' className='mt-4'>
            ...
            <div className='space-y-6'>
              ...
              {allUserAddresses.isLoading && (<div className='flex items-center justify-center py-8'>
                  <div className='text-center'>
              ...
              {allUserAddresses.error && (<Alert variant='destructive'>
                  <AlertDescription>Error: {allUserAddresses.error.message}</AlertDescription>
                </Alert>)}
              
              {!!allUserAddresses.data && allUserAddresses.data.length > 0 && !allUserAddresses.isLoading ? (<div className='grid gap-4'>
                  {allUserAddresses.data.map((address) => (<Card key={address.id} className='p-6 hover:shadow-md transition-shadow'>
                ...
                <div className='space-y-4'>
                          ...
                          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                            <div className='space-y-2'>
                              ...
                              <Input id={`edit-contactName-${address.id}`} value={editFormData.contactName} onChange={(e) => setEditFormData(prev => ({ ...prev, contactName: e.target.value }))} className='h-10'/>
                            </div>
                            <div className='space-y-2'>
                              <Label htmlFor={`edit-companyName-${address.id}`} className='text-sm font-medium'>Company Name</Label>
                              <Input id={`edit-companyName-${address.id}`} value={editFormData.companyName} onChange={(e) => setEditFormData(prev => ({ ...prev, companyName: e.target.value }))} className='h-10'/>
                            </div>
                          </div>
                          
                          <div className='space-y-2'>
                            <Label htmlFor={`edit-addressLine1-${address.id}`} className='text-sm font-medium'>Address Line 1 *</Label>
                            <Input id={`edit-addressLine1-${address.id}`} value={editFormData.addressLine1} onChange={(e) => setEditFormData(prev => ({ ...prev, addressLine1: e.target.value }))} className='h-10'/>
                          </div>
                          
                          <div className='space-y-2'>
                            <Label htmlFor={`edit-addressLine2-${address.id}`} className='text-sm font-medium'>Address Line 2</Label>
                            <Input id={`edit-addressLine2-${address.id}`} value={editFormData.addressLine2} onChange={(e) => setEditFormData(prev => ({ ...prev, addressLine2: e.target.value }))} className='h-10'/>
                          </div>
                          
                          <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                            <div className='space-y-2'>
                              <Label htmlFor={`edit-city-${address.id}`} className='text-sm font-medium'>City *</Label>
                              <Input id={`edit-city-${address.id}`} value={editFormData.city} onChange={(e) => setEditFormData(prev => ({ ...prev, city: e.target.value }))} className='h-10'/>
                            </div>
                            <div className='space-y-2'>
                              <Label htmlFor={`edit-state-${address.id}`} className='text-sm font-medium'>State *</Label>
                              <Input id={`edit-state-${address.id}`} value={editFormData.state} onChange={(e) => setEditFormData(prev => ({ ...prev, state: e.target.value }))} className='h-10'/>
                            </div>
                            <div className='space-y-2'>
                              <Label htmlFor={`edit-postalCode-${address.id}`} className='text-sm font-medium'>Postal Code *</Label>
                              <Input id={`edit-postalCode-${address.id}`} value={editFormData.postalCode} onChange={(e) => setEditFormData(prev => ({ ...prev, postalCode: e.target.value }))} className='h-10'/>
                            </div>
                          </div>
                          ...
                          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                            <div className='space-y-2'>
                              ...
                              <Select value={editFormData.country} onValueChange={(value) => setEditFormData(prev => ({ ...prev, country: value }))}>
                                <SelectTrigger className='h-10'>
                                  <SelectValue placeholder="Select country"/>
                                </SelectTrigger>
                                <SelectContent>
                            ...
                            <div className='space-y-2'>
                              ...
                              <Select value={editFormData.addressType} onValueChange={(value) => setEditFormData(prev => ({ ...prev, addressType: value }))}>
                                <SelectTrigger className='h-10'>
                                  <SelectValue placeholder="Select type"/>
                                </SelectTrigger>
                                <SelectContent>
                          ...
                          <div className='space-y-2'>
                            ...
                            <Input id={`edit-label-${address.id}`} value={editFormData.label} onChange={(e) => setEditFormData(prev => ({ ...prev, label: e.target.value }))} placeholder="e.g., Home, Office, Client ABC" className='h-10'/>
                          </div>
                ...
                <div className='flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4'>
                          <div className="flex-1 min-w-0 space-y-3">
                            ...
                            <div className='flex items-center gap-3 flex-wrap'>
                              ...
                              {address.isDefault && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200">
                                  Default Address
                                </span>)}
                            ...
                            {/* Company name */}
                            {address.companyName && (<p className='text-muted-foreground font-medium'>{address.companyName}</p>)}
                            ...
                            <div className='flex gap-2 flex-wrap'>
                              {address.label && (<span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-purple-100 text-purple-700 border border-purple-200">
                                  📍 {address.label}
                                </span>)}
                              {address.usageCount > 0 && (<span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-700 border border-green-200">
                          ...
                          <div className='flex gap-2 shrink-0'>
                            <Button variant='outline' size='sm' className='h-9' onClick={() => handleEditClick(address)}>
                              Edit
                            </Button>
                            <Button onClick={() => handleDelete(address.id)} variant='destructive' size='sm' className='h-9'>
                              Delete
                            </Button>
                          </div>
                        </div>)}
                    </Card>))}
                </div>) : !allUserAddresses.isLoading && (<Card className='p-8 text-center'>
                  <div className='space-y-4'>
...
//# sourceMappingURL=AddressManagementPage.jsx.map