...
export async function healthCheckEndpoint(req: any, res: any, context: any) {
  try {
    ...
    if (path === '/health/simple') {
      // Simple health check for basic monitoring - optimized for deployment
      const health = simpleHealthCheck();
      res.status(health.status === 'ok' ? 200 : 500).json(health);
    } else if (path === '/health/detailed') {
      // Detailed health check with system metrics - only after deployment
      try {
        ...
        // Fallback to simple check if detailed check fails
        const health = simpleHealthCheck();
        res.status(health.status === 'ok' ? 200 : 500).json({
          ...health,
          note: 'Detailed check failed, using simple check'
        });
      }
    } else {
      ...
      try {
        ...
        // Fallback to simple check if comprehensive check fails
        const health = simpleHealthCheck();
        res.status(health.status === 'ok' ? 200 : 500).json({
          ...health,
          note: 'Comprehensive check failed, using simple check'
        });
      }
    }
  } catch (error) {
